...
export const ADD_PROFESSIONAL = async (decryptedBody) => {
    const { data } = decryptedBody;
    const intervals = convertToIntervals(data.professionalWorkingTime);
    const days = data.professionalWorkingDays.map(day => dayNameToNumberMapEnglish[day]);


    const { data: professionalData, error: professionalError } = await supabase
        .from('professionals')
        .insert({ name: data.professionalName })
        .select()
        .single(); // .single() retorna um objeto em vez de array
